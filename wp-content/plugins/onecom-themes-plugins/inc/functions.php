...
if( ! function_exists( 'onecom_install_plugin_callback' ) ) {
	function onecom_install_plugin_callback( $isAjax = true, $pluginSlugParam = '' ) {
		...
		if( $result == true ) {
            ...
            (class_exists('OCPushStats')?\OCPushStats::push_stats_event_themes_and_plugins('install','plugin',$plugin_slug,'plugins_page'):'');
			...
			if( $redirect == false || $redirect == '' || is_multisite() ) {
				$button_html = '<a href="'.$activateUrl.'" class="button button-primary activate-plugin">'.__( 'Activate', OC_PLUGIN_DOMAIN ).'</a>';
			} else {
				$button_html = '<a class="activate-plugin activate-plugin-ajax button button-primary" href="javascript:void(0)" data-action="onecom_activate_plugin" data-redirect="'.$redirect.'" data-slug="'.$status['file'].'" data-name="'.$plugin_name.'">'.__( 'Activate', OC_PLUGIN_DOMAIN ).'</a>';
			}
			$response[ 'button_html' ] = $button_html;
	...
	function onecom_generic_promo() {
		...
		if( ! $is_transient ) {
			...
			if( ! is_wp_error( $response ) ) {
				...
				$x_promo_transient = wp_remote_retrieve_header( $response, 'X-ONECOM-Transient' );
		...
		if( ! empty( $plugins_to_install ) ) {
			...
			foreach ( $plugins_to_install as $pluginSlug => $pluginFile ) {
				...
				if( FALSE === is_dir( $plugins_dir_path . DIRECTORY_SEPARATOR . $pluginSlug ) ) {
					...
					$response = onecom_install_plugin_callback(  false, $pluginSlug );
					if(false !== $response){
